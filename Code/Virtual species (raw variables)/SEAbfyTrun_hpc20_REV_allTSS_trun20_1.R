...
#Select truncation
TrunAmount <- 0.2 #0/0.1/0.2/0.3, portion of data to truncate
Trunarea <- "SOUTH" # SOUTH/NORTH

#Model run name
datetime <- "bmod424_trunc_rev"
...
model.files.dir <- paste0("results_",datetime,"_",Trunarea,"_trun",TrunPrec,"perc")
dir.create(paste0("/lustre1/g/sbs_bonebrake/Eugene/SDMout/",model.files.dir)) 
species.list.dir <- paste0("species_list_",datetime,"_",Trunarea,"_trun",TrunPrec,"perc")
...
#m <- 1:ceiling(length(Species.list)/2)
#m <- floor(length(Species.list)/2):length(Species.list)
...
MyBiomodSF <- function(m) {
  try({
    ...
    #Truncate presence data sampling area
    if (Trunarea == "SOUTH") {
      Obs_df <- Obs_df[order(Obs_df$y, decreasing = TRUE), ]
    } else if (Trunarea == "NORTH") {
      Obs_df <- Obs_df[order(Obs_df$y, decreasing = FALSE), ]
    } else {message("Please specify area to truncate for ", Species.list[[m]])}
    
    Obs_df <- Obs_df[1:floor(nrow(Obs_df)*(1-TrunAmount)),]
    
    #Truncate pseudo-absence data sampling area
    truncationfront <- Obs_df[nrow(Obs_df),2]
    if (Trunarea == "SOUTH") {
      current.climate.trun <- terra::crop(current.climate,ext(6657553.33731186,15277553.3373119,truncationfront,4300563.25646216))
    } else if (Trunarea == "NORTH") {
      truncationfront.adjusted <- truncationfront+10000 #Need adjustment to accurately crop
      current.climate.trun <- terra::crop(current.climate,ext(6657553.33731186,15277553.3373119,0,truncationfront.adjusted))
    }
    ...
    if (nrow(df) > 20) { #GBM works with at least 21 records
      ...
      #Ensemble modelling
      try({myBiomodEM00 <- BIOMOD_EnsembleModeling(bm.mod = myBiomodModelOut,
                                                   models.chosen = "all",
                                                   em.by = "all", 
                                                   metric.select = c("TSS"),
                                                   metric.select.thresh = 0,
                                                   em.algo = c('EMmean', 'EMca', 'EMmedian', 'EMwmean')) 
      
      #Single models individually estimates habitat suitability (Projection)
      myBiomodProj00 <- BIOMOD_Projection(
      ...
      if (length(myBiomodEM00@em.models_kept) >= 1) {
        
        myBiomodEF <- BIOMOD_EnsembleForecasting(
          bm.em = myBiomodEM00,
          bm.proj = myBiomodProj00,
          models.chosen = 'all',
          metric.binary = "TSS")
      ...
      #Ensemble modelling
      try({myBiomodEM03 <- BIOMOD_EnsembleModeling(bm.mod = myBiomodModelOut,
                                                   models.chosen = "all",
                                                   em.by = "all", 
                                                   metric.select = c("TSS"),
      ...
      #Single models individually estimates habitat suitability (Projection)
      myBiomodProj03 <- BIOMOD_Projection(
        bm.mod = myBiomodModelOut,
        new.env = current.climate,
        proj.name = "TSS03",
        models.chosen = "all",
      ...
      if (length(myBiomodEM03@em.models_kept) >= 1) {
        
        myBiomodEF <- BIOMOD_EnsembleForecasting(
          bm.em = myBiomodEM03,
          bm.proj = myBiomodProj03,
          models.chosen = 'all',
          metric.binary = "TSS")
      ...
      #Ensemble modelling
      try({myBiomodEM05 <- BIOMOD_EnsembleModeling(bm.mod = myBiomodModelOut,
                                                   models.chosen = "all",
                                                   em.by = "all", 
                                                   metric.select = c("TSS"),
      ...
      #Single models individually estimates habitat suitability (Projection)
      myBiomodProj05 <- BIOMOD_Projection(
        bm.mod = myBiomodModelOut,
        new.env = current.climate,
        proj.name = "TSS05",
        models.chosen = "all",
      ...
      if (length(myBiomodEM05@em.models_kept) >= 1) {
        
        myBiomodEF <- BIOMOD_EnsembleForecasting(
          bm.em = myBiomodEM05,
          bm.proj = myBiomodProj05,
          models.chosen = 'all',
          metric.binary = "TSS")
      ...
      #Ensemble modelling
      try({myBiomodEM07 <- BIOMOD_EnsembleModeling(bm.mod = myBiomodModelOut,
                                                   models.chosen = "all",
                                                   em.by = "all", 
                                                   metric.select = c("TSS"),
      ...
      #Single models individually estimates habitat suitability (Projection)
      myBiomodProj07 <- BIOMOD_Projection(
        bm.mod = myBiomodModelOut,
        new.env = current.climate,
        proj.name = "TSS07",
        models.chosen = "all",
      ...
      if (length(myBiomodEM07@em.models_kept) >= 1) {
        
        myBiomodEF <- BIOMOD_EnsembleForecasting(
          bm.em = myBiomodEM07,
          bm.proj = myBiomodProj07,
          models.chosen = 'all',
          metric.binary = "TSS")
