...
model.files.dir <- paste0("results_",runID,"_",Trunarea,"_trun",TrunPrec,"perc")
dir.create(paste0("/lustre1/g/sbs_bonebrake/Eugene/SDMout/",model.files.dir)) 
species.list.dir <- paste0("species_list_",runID,"_",Trunarea,"_trun",TrunPrec,"perc")
  ...
  try({
    ...
    if (nrow(df) > 19) { #GBM works with at least 20 records
      ...
      bm.tuning <- BIOMOD_Tuning(
        ...
        ME.env = current.climate.trun)
      myBiomodOption <- bm.tuning$models.options
      
      myBiomodModelOut <- BIOMOD_Modeling( 
        bm.format = myBiomodData, 
        models = c("GLM","GBM","MARS","RF"), 
